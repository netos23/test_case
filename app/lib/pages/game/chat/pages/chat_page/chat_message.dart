...
typedef LinkCallback = void Function(String, bool);
...
class TimestampedChatMessageRenderObject extends RenderBox {
  ...
  late LinkCallback? _onLink;
  late TextDirection _textDirection;
  late String _text;
  late ChatMessageLink? _link;
  late String _sentAt;
  ...
  LinkCallback? get onLink => _onLink;

  set onLink(LinkCallback? onLink) {
    _onLink = onLink;
  }
  ...
  ChatMessageLink? get link => _link;

  set link(ChatMessageLink? val) {
    _link = val;
    _textPainter.text = textTextSpan;
    markNeedsLayout();
    // Because changing any text in our widget will definitely change the
  ...
  TextSpan get textTextSpan {
    ...
    if (link == null) {
      return TextSpan(text: _text, style: _textStyle);
    }

    String first = _text.substring(0, link.start);
    String second = _text.substring(link.start, link.end);
    String third = _text.substring(link.end);
    return TextSpan(
      children: [
        TextSpan(
          text: first,
        ...
        TextSpan(
          ...
          recognizer: TapGestureRecognizer()
            ..onTap = () {
              onLink?.call(second, link.external);
            },
        ),
        TextSpan(
          text: third,
